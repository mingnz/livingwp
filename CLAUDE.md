...
- **`packages/article-contract`** (`@livingwp/article-contract`) — the shared
  frontmatter contract (a Zod-shape factory) consumed by both of the above, so
  the field definitions cannot drift between writer and reader.
