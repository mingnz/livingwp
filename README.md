...
- Pin one article to a specific model via `research_model`. Bare model names use OpenAI; prefix with `anthropic/` or `google/` to use those providers. Leave the key out to use the default model for every article.
...
The agent is built on the [Vercel AI SDK](https://ai-sdk.dev), so it works with any supported provider. The default research model tracks a current frontier model and is set by the `RESEARCH_MODEL` environment variable (a repository variable of that name in GitHub Actions), with a fallback default in `src/agent/src/agent.ts`. Each provider's hosted web-search tool is used automatically based on the resolved provider.
